...
def fase1():
    ...
    while True:
        try:
            ...
            t = int(
                input(f"Tentativa (entre {menor_palpite} e {maior_palpite}): "))
            if t <= 100 and t >= 1:
                if n < t:
                    print(menor)
                    print(f"O número é menor que {t}")
                    maior_palpite = t - 1
                    rt += 1
                elif t < n:
                    print(maior)
                    print(f"O número é maior que {t}")
                    menor_palpite = t + 1
                    rt += 1
                else:
                    print(acerto)
...
def fim():
    ...
    d = input(
        "Dejesa tentar novamente? Digite (s) para sim e (n) para não: ").strip()
    if d.lower() == "s":
        menu()
    elif d.lower() == "n":
